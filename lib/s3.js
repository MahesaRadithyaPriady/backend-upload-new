...
export function getS3() {
  ...
  const region = config.B2_S3_REGION || process.env.B2_S3_REGION || 'us-east-1';
  const accessKeyId = ensureEnv('B2_S3_ACCESS_KEY_ID');
  const secretAccessKey = config.B2_S3_SECRET_ACCESS_KEY || process.env.B2_S3_SECRET_ACCESS_KEY || process.env.B2_S3_SECRET_APPLICATION_KEY;
  if (!secretAccessKey) {
