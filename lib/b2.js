...
function isB2TransientUploadError(err) {
  ...
  return (
    ...
    status === 503 ||
    status === 500 ||
    (Number.isFinite(Number(status)) && Number(status) >= 500) ||
    code === 'service_unavailable' ||
    code === 'ERR_BAD_RESPONSE' ||
    /no tomes available|service_unavailable|too many requests|rate limit|incident id|bad response|econnreset|etimedout|socket hang up/i.test(lowerMsg)
  );
}

async function withB2TransientRetry(fn, { retries = 3, baseDelayMs = 600, label } = {}) {
  let lastErr;
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const debug =
        Boolean(config.DEBUG_SIGNED_URL) ||
        process.env.DEBUG_B2_UPLOAD === 'true' ||
        process.env.DEBUG_B2_UPLOAD === '1';
      if (debug) {
...
async function withB2AuthRetry(fn) {
  try {
    ...
    if (Boolean(config.DEBUG_SIGNED_URL)) {
...
function ensureEnv(name) {
  const v = config[name] ?? process.env[name];
  if (!v) {
...
export async function getB2() {
  ...
  const bucketName = config.B2_BUCKET_NAME || process.env.B2_BUCKET_NAME || '';
...
export async function uploadBuffer({ fileName, buffer, contentType }) {
  ...
  return withB2TransientRetry(exec, { retries, baseDelayMs, label: fileName });
}
...
async function streamToTempFileAndSha1(stream) {
  ...
  const debug = process.env.DEBUG_B2_UPLOAD === 'true' || process.env.DEBUG_B2_UPLOAD === '1';
  const startedAt = Date.now();
  if (debug) {
    console.log('[b2] stream->temp start', { tmpDir, filePath });
  }
  ...
  try {
    ...
    if (debug) {
      console.log('[b2] stream->temp done', { size, sha1, durationMs: Date.now() - startedAt });
    }
...
async function uploadFileFromPath({ fileName, filePath, contentType, sha1, contentLength }) {
  ...
  const exec = async () => {
    ...
    const startedAt = Date.now();
    if (debug) {
      console.log('[b2] uploadFile start', {
        fileName,
        filePath,
        contentLength: len,
    ...
    if (debug) {
      console.log('[b2] uploadFile done', {
        fileName,
...
export async function uploadFromStream({ fileName, stream, contentType, expectedSizeBytes } = {}) {
  ...
  const startedAt = Date.now();
  if (debug) {
    console.log('[b2] uploadFromStream called', {
      fileName,
      contentType: contentType || 'application/octet-stream',
      expectedSizeBytes: Number.isFinite(Number(expectedSizeBytes)) ? Number(expectedSizeBytes) : null,
    });
  }
  ...
  if (!shouldUseTempFile) {
    ...
    const res = await uploadBuffer({ fileName, buffer, contentType });
    if (debug) {
      console.log('[b2] uploadFromStream done (buffer)', { fileName, durationMs: Date.now() - startedAt, size: buffer.length });
    }
    return res;
  }
  ...
  try {
    const res = await uploadFileFromPath({ fileName, filePath, contentType, sha1, contentLength: size });
    if (debug) {
      console.log('[b2] uploadFromStream done (tempfile)', { fileName, durationMs: Date.now() - startedAt, size });
    }
    return res;
  } finally {
    try {
...
export async function downloadByName({ fileName, range }) {
  ...
  const res = await withB2AuthRetry(() =>
    b2.downloadFileByName({
      bucketName: config.B2_BUCKET_NAME || process.env.B2_BUCKET_NAME,
      fileName,
      headers,
      responseType: 'stream',
    }),
...
export async function getSignedDownloadUrl({ fileName, validDurationInSeconds } = {}) {
  ...
  if (!bucketName) {
   ...
   const ttlFromEnv = Number((config.B2_SIGNED_URL_TTL ?? process.env.B2_SIGNED_URL_TTL) || '0');
