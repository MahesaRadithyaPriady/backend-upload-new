...
async function uploadFileFromPath({ fileName, filePath, contentType, sha1 }) {
...
export async function uploadFromStream({ fileName, stream, contentType, expectedSizeBytes } = {}) {
  ...
  const { tmpDir, filePath, sha1 } = await streamToTempFileAndSha1(stream);
  try {
    return await uploadFileFromPath({ fileName, filePath, contentType, sha1 });
  } finally {
    try {
